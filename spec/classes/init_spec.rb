  ...
  on_supported_os.each do |os, facts|
    ...
    context "When specifying restart_service" do
      ...
      it  "should have network_interfaces notifying the network service" do
        [ 'ens39', 'ens99', 'ens51', 'ens52' ].each do |int|
          is_expected.to contain_network_interface(int).that_notifies('Service[NetworkManager]')
      ...
      it "should have ip_allocations notifying the network service" do
        [ '10.9.1.10', '10.7.6.10', '10.1.1.1' ].each do |ip|
          is_expected.to contain_ip_allocation(ip).that_notifies('Service[NetworkManager]')
        end
      end
    end
    ...
    context "When specifying restart_interface" do
      ...
      it "should have network_interfaces notifying the interface service" do
        [ 'ens39', 'ens99', 'ens51', 'ens52' ].each do |int|
          is_expected.not_to contain_network_interface(int).that_notifies('Service[NetworkManager]')
      ...
      it "should not notify the network service from ip allocations" do
        is_expected.not_to contain_ip_allocation('10.9.1.10').that_notifies("Service[NetworkManager]")
        is_expected.not_to contain_ip_allocation('10.7.6.10').that_notifies("Service[NetworkManager]")
        is_expected.not_to contain_ip_allocation('10.1.1.1').that_notifies("Service[NetworkManager]")
      end
    end
